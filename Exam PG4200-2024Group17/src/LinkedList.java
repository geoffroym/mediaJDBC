...
public class LinkedList {
    ...
    public cityNode mergeSort(cityNode start) { //static
        ...
        cityNode left = mergeSort(start);
        cityNode right = mergeSort(nextOfMiddle);
        cityNode sortedList = merge(left, right);
        return sortedList;
    }
    public cityNode merge(cityNode left, cityNode right) {
    ...
    public  cityNode mergeSortLat(cityNode start) { //static
        if (start == null || start.next == null) {
            return start;
        }
        cityNode middle = getMiddle(start);
        ...
        cityNode left = mergeSortLat(start);
        cityNode right = mergeSortLat(nextOfMiddle);
        cityNode sortedList = mergeLat(left, right);
        return sortedList;
    }

    public cityNode mergeLat(cityNode left, cityNode right) {
        cityNode pointer = new cityNode(null);
        cityNode tail = pointer;
        while (left != null && right != null) {
    ...
    public cityNode getMiddle(cityNode start) {
        if (start == null) {
            return start;
        }
        cityNode slow = start;
    ...
    public LinkedList read(String file) {
        ...
        try (CSVReader reader = new CSVReader(new FileReader(file))) {
            ...
            while ((nextLine = reader.readNext()) != null ){
                ...
                int id = Integer.parseInt(nextLine[10]);
